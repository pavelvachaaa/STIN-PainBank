...
const ModalButton = () => {
    ...
    return (
        <button type="button" onClick={() => toggleModal({ type: "OUT", currency: "CZK" })} className="hidden sm:inline-flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-xs px-3 py-1.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800 mr-2 "><svg aria-hidden="true" className="mr-1 -ml-1 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clipRule="evenodd"></path></svg> Nová platba</button>
    )
}
...
const Header = () => {
    ...
    return (
        <Navbar className="px-4 lg:px-6 py-2.5 overflow-x-hidden" fluid={true} rounded={true} >
            <div className="flex justify-start items-center">
                <Navbar.Brand href="/dashboard">
                    <img src="https://flowbite.com/docs/images/logo.svg" className="mr-3 h-6 sm:h-9" alt="PainBank Logo" />
                    <span className="self-center whitespace-nowrap text-xl font-semibold dark:text-white"> PainBank </span>
                </Navbar.Brand>
            ...
            <div className="flex">
                ...
                <Dropdown arrowIcon={false} inline={true} className="relative" label={<Avatar alt="User settings" img="https://flowbite.com/docs/images/people/profile-picture-5.jpg" rounded={true}> </Avatar>} >
                    ...
                    <Dropdown.Item>
                        <Link href={"/dashboard/payments"}>Pohyby</Link>
                    </Dropdown.Item>
                    <Dropdown.Divider />
